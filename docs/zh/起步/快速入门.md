...
如果你不是特别理解上面的话，不用在意，因为你完全不需要关注 widget 的构建过程，Ribir 也禁止干涉这个过程。你只需明白，Ribir 将所有的 widget 分成四类，和你打交道的正是它们：
...
尽管 `rdl!` 支持任意 Rust 表达式，但我们所说的声名式创建对象，特指通过结构体字面量的方式。
...
在组合 widget 的过程中，用到了大量的 `rdl!`，这看上去太冗长了，无法让你一眼看到重点信息。但另一方面，它又让你在与 Rust 语法交互的例子中（特别是复杂的例子）能有一个清晰的声名式结构——当你看到 `rdl!` 时，你就知道一个 widget 节点的组合或创建开始了。
...
如果紧根治 `$xxx` 调用 `write()` 或 `silent()`，这表示对状态的数据进行写引用。此时：

- `$xxx.write()` 即是 `xxx.write()` 
- `$xxx.silent()` 即是 `xxx.silent()`

`write` 和 `silent` 的差别在于，通过 `write` 返回的写引用修改数据会触发视图更新，而 `silent` 则不会触发。

如果 `$xxx` 既没有调用 `write()`，也没有调用 `silent()`，则表示对状态的进行读引用。此时:
...
到目前为止，你所有创建的 widget 的视图结构都是静态的，它们或者属性会随着数据变更，但 widget 的结构不会随着数据而变更。实际上，你同样可以通过 `Pipe` 流来创建持续变化的 widget 结构。
...
拿 `Margin` 举例，假设你要为一个 `Text` 设置 10 像素的空白边距，你可以：
...
fn main() {
  App::run(fn_widget! {
    // 没有声明 `margin`
    let hello_world = @Text { text: "Hello World!" };
    // 直接修改 `margin`
...
// 根状态的源状态是它自己
let _: Reader<AppData> = state.origin_reader();
let _: Writer<AppData> = state.origin_writer();

// 子状态的源状态是它的父亲
let _: Reader<AppData> = split_count.origin_reader();
