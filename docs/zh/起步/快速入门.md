...
如果 `$state` 既没有 `write()` 跟随，也没有 `silent()` 跟随，则表示对状态的进行读引用。此时:
...
拿 `Margin` 举例，假设你要为一个 `Text` 设置 `Margin`：
...
当你通过声明式创建了一个 widget 后，你可以直接访问内建 widget 的字段，即使你并没有显示声明它们（如果你在代码中用到它们，响应的内建 widget 会被创建）。比如：
...
**分离**是从一个父状态中分离出子状态，子状态不仅缩减了数据的可见范围，同时拥有了一个独立的子修改通知范围。通过子状态修改数据，只会通知子状态的依赖；通过父状态的修改数据，会同时通知父子状态的依赖。

你要注意的是，不论是转换还是分离，父子状态共享的都是同一份数据。因此，父子状态修改的是同一份数据，但它们触发的依赖更新的范围可能不同。
...
在上面的例子中，父子状态的通知顺序是不确定的，因此你可能看到不一样的打顺序。因为 Ribir 的数据修改通知是批量发出的，所以每次数据修改都调用了 `AppCtx::run_until_stalled()` 来方便你理解，但这不应该出现在你真实的代码中。
