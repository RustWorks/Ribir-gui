  ...
  match event.key {
    VirtualKeyCode::Left => {
      if is_move_by_word(event) {
        ...
        this.caret = this.helper.prev_cluster(this.caret.caret_position()).into();
      }
    }
    VirtualKeyCode::Right => {
      if is_move_by_word(event) {
        ...
        this.caret = this.helper.next_cluster(this.caret.caret_position()).into();
      }
    }
    VirtualKeyCode::Up => {
      this.caret = this.helper.up_cluster(this.caret.caret_position()).into();
    }
    VirtualKeyCode::Down => {
      this.caret = this.helper.down_cluster(this.caret.caret_position()).into();
    }
